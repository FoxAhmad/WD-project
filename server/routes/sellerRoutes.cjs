 ...
 */
router.get('/bookings', authenticateUser, authorizeRoles('seller'), async (req, res) => {
  ...
  try {
    ...
    if (!bookings.length) {
      return res.status(404).json({ message: 'No bookings found for this seller.' });
    }

    return res.status(200).json(bookings);
  } catch (error) {
    console.error(`Error fetching bookings for seller ${sellerId}:`, error);
    return res.status(500).json({ message: 'Failed to fetch bookings. Please try again later.' });
  }
});
